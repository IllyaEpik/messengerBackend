...
export const chatService: IChatService = {
	...
	updateChat: async (data, chatId, avatar) => {
		try {
			...
			if (removeParticipants) {
				await chatRepository.deleteSelectedParticipants(
					removeParticipants,
					BigInt(chatId),
				);
			}
	...
	isUserInChat: async (chatId, userId) => {
		try {
			const isIn = await chatRepository.isUserInChat(
				BigInt(chatId),
				BigInt(userId),
			);
			return isIn;
	...
	async getChatByContact(userId, friendId) {
		try {
			let chat = await chatRepository.getChatByUsers(
				BigInt(userId),
				BigInt(friendId),
			);
	...
	async deleteChat(userId, chatId) {
		const chat = await chatRepository.deleteChat(
			BigInt(userId),
			BigInt(chatId),
		);
		if (!chat) return "chat is not found|404";
