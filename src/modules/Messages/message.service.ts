...
export const messageService: IMessageService = {
    create: async (data, senderId) => {
        try {
            ...
            console.log(data, data.text, "dsaasdasdasdsadsadasdsadsdsadasdasdasdasdasd")
    ...
    update: async (messageId, text, senderId) => {
        try {
            ...
            const message = await messageRepository.update(BigInt(messageId), { text });
            return message;
        } catch (error) {
            return "not found|404";
        }
    ...
    getByChat: async (chatId, skip = 0, take = 20) => {
        try {
            const messages = await messageRepository.getByChat(BigInt(chatId), skip, take);
            return messages;
        } catch (error) {
            return "not found|404";
        }
    ...
    addReader: async (messageId, userId) => {
        try {
            const message = await messageRepository.addReader(BigInt(messageId), BigInt(userId));
            return message;
        } catch (error) {
            return "not found|404";
        }
