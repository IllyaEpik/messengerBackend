...
export type IMessageRepository = {
    ...
    getByChat: (chatId: bigint, skip?: number, take?: number) => Promise<IMessage[]>;
    addReader: (messageId: bigint, userId: bigint) => Promise<IMessage>;
};

export type IMessageService = {
    create: (data: IMessageCreateInput, senderId: number) => Promise<IMessage | string>;
    update: (messageId: number, text: string, senderId: number) => Promise<IMessage | string>;
    getByChat: (chatId: number, skip?: number, take?: number) => Promise<IMessage[] | string>;
